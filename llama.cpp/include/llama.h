...
extern "C" {
    ...
    // Fast recurrent state checkpointing for speculative decoding with hybrid models.
    // Save/restore recurrent state only (attention KV cache is not affected).
    // Returns true on success. Only works on hybrid memory (llama_memory_hybrid).
...
#define LLAMA_STATE_SEQ_FLAGS_PARTIAL_ONLY 1
    ...
    // Enable/disable MTP hidden state extraction during llama_decode()
    LLAMA_API void llama_set_mtp_enabled(struct llama_context * ctx, bool enabled);

    // Run the MTP layer to produce draft logits from the last decode's hidden state
    // id_last: the token sampled from the main model's output
    // Returns 0 on success
    LLAMA_API int32_t llama_decode_mtp(struct llama_context * ctx, llama_token id_last);

    // Get the MTP draft logits from the last call to llama_decode_mtp()
    LLAMA_API float * llama_get_logits_mtp(struct llama_context * ctx);

    // After speculative verification, set the correct hidden state output index
    // idx: 0-based index into the batch's output positions
    LLAMA_API void llama_set_mtp_output_index(struct llama_context * ctx, int32_t idx);
