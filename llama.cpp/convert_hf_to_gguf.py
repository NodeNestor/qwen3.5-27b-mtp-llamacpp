...
class Qwen3_5TextModel(_LinearAttentionVReorderBase):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self._mtp_layers = self.hparams.get("mtp_num_hidden_layers", 0)
        if self._mtp_layers > 0:
            self.block_count = self.hparams["num_hidden_layers"] + self._mtp_layers
            self.tensor_map = gguf.get_tensor_name_map(self.model_arch, self.block_count)
    ...
    def modify_tensors(self, data_torch: Tensor, name: str, bid: int | None) -> Iterable[tuple[str, Tensor]]:
        if name.startswith("mtp."):
            if self._mtp_layers == 0:
                return  # skip if no MTP support configured

            n_main = self.hparams["num_hidden_layers"]

            if name.find("layers.") != -1:
                # `mtp.layers.{bid}.*` → `model.layers.{bid + n_main}.*`
                name = name.replace(f"mtp.layers.{bid}", f"model.layers.{bid + n_main}")
            else:
                # MTP-specific tensors: fc, norms
                from pathlib import Path
                remapper = {
                ...
                if key not in remapper:
                    logger.warning(f"Unknown MTP tensor: {name}, skipping")
                    return
                new_name = remapper[key] + _n.suffix

                # Set shared weights for all MTP layers
                for mtp_bid in range(n_main, self.block_count):
                    yield from super().modify_tensors(data_torch, new_name.format(bid=mtp_bid), mtp_bid)
                return

            # For MTP layer tensors, apply norm +1 fix and other transforms
            if name.endswith("norm.weight") and not name.endswith("linear_attn.norm.weight"):
                data_torch = data_torch + 1

            yield from super(_LinearAttentionVReorderBase, self).modify_tensors(data_torch, name, bid + n_main)
            return

        # Non-MTP tensors: delegate to parent
        yield from super().modify_tensors(data_torch, name, bid)
