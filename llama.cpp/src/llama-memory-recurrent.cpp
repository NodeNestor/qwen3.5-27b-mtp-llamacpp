...
bool llama_memory_recurrent::seq_rm(llama_seq_id seq_id, llama_pos p0, llama_pos p1) {
    ...
    if (0 <= seq_id) {
        ...
        if (tail_id >= 0) {
            ...
            // partial intersection including the final pos
            // Allow it by adjusting position - enables speculative decoding with hybrid models
            // The recurrent state data becomes stale but the attention KV cache remains correct,
            // and subsequent tokens will gradually correct the recurrent state
            if (0 < p0 && p0 <= cell.pos && p1 > cell.pos) {
                LLAMA_LOG_DEBUG("%s: partial seq_rm (pos %d -> %d) - recurrent state may be stale\n",
                    __func__, (int)cell.pos, (int)(p0 - 1));
...
//
// fast checkpoint for speculative decoding
//

void llama_memory_recurrent::set_checkpoint_backends(const std::vector<ggml_backend_t> & backends) {
    ckpt.backends = backends;
}

// Find the backend that owns a tensor's buffer
static ggml_backend_t find_backend_for_tensor(const ggml_tensor * tensor, const std::vector<ggml_backend_t> & backends) {
    ...
    for (auto * backend : backends) {
        ...
        if (dev && ggml_backend_dev_buffer_type(dev) == buft) {
            return backend;
        }
    }
    return nullptr;
...
void llama_memory_recurrent::checkpoint_alloc() {
    if (ckpt.initialized) {
        return;
    }
    ...
    ckpt.r_ckpt.resize(n_layer, nullptr);
    ckpt.s_ckpt.resize(n_layer, nullptr);

    // Group layers by buffer type (same approach as constructor)
    struct ggml_backend_buft_comparator {
        bool operator()(const ggml_backend_buffer_type_t & lhs, const ggml_backend_buffer_type_t & rhs) const {
            return strcmp(ggml_backend_buft_name(lhs), ggml_backend_buft_name(rhs)) < 0;
        }
    ...
    for (int32_t il = 0; il < n_layer; ++il) {
        if (r_l[il] != nullptr) {
            ...
            ggml_context * ctx = ctx_for_buft(buft);
            ggml_tensor * r = ggml_dup_tensor(ctx, r_l[il]);
            ggml_format_name(r, "ckpt_r_l%d", il);
            ckpt.r_ckpt[il] = r;
            total_bytes += ggml_nbytes(r_l[il]);
        }
        if (s_l[il] != nullptr) {
            ggml_backend_buffer_type_t buft = ggml_backend_buffer_get_type(s_l[il]->buffer);
            ggml_context * ctx = ctx_for_buft(buft);
            ggml_tensor * s = ggml_dup_tensor(ctx, s_l[il]);
            ggml_format_name(s, "ckpt_s_l%d", il);
            ckpt.s_ckpt[il] = s;
            total_bytes += ggml_nbytes(s_l[il]);
        }
    }

    // Allocate buffers on the same backend buffer types as the source tensors
    for (auto & [buft, ctx] : ctx_map) {
        ggml_backend_buffer_t buf = ggml_backend_alloc_ctx_tensors_from_buft(ctx.get(), buft);
        if (!buf) {
            LLAMA_LOG_ERROR("%s: failed to allocate GPU checkpoint buffer\n", __func__);
            return;
        }
        ggml_backend_buffer_clear(buf, 0);
        ckpt.ckpt_bufs.emplace_back(std::move(ctx), buf);
    }

    ckpt.initialized = true;

    LLAMA_LOG_INFO("%s: allocated %.2f MiB for GPU recurrent checkpoint\n", __func__, (float)total_bytes / (1024.0f * 1024.0f));
}
...
bool llama_memory_recurrent::checkpoint_save(llama_seq_id seq_id) {
    ...
    const int32_t tail_id = cells[seq_id].tail;
    if (tail_id < 0) {
        // no state to save (sequence hasn't been used yet)
        ckpt.cell_idx = -1;
        ckpt.valid = true;
        return true;
    }
    ...
    ckpt.src = cell.src;
    ckpt.tail = tail_id;
    ckpt.seq_ids = cell.seq_id;
    ckpt.cell_idx = tail_id;
    ckpt.saved_head = head;
    ckpt.saved_used = used;

    // save tensor data using GPU-to-GPU async copies
    const int32_t n_layer = hparams.n_layer;

    for (int32_t il = 0; il < n_layer; ++il) {
        if (r_l[il] != nullptr && ckpt.r_ckpt[il] != nullptr) {
            ggml_backend_t backend = find_backend_for_tensor(r_l[il], ckpt.backends);
            if (backend) {
                ggml_backend_tensor_copy_async(backend, backend, r_l[il], ckpt.r_ckpt[il]);
            } else {
                // fallback to sync copy
                ggml_backend_tensor_copy(r_l[il], ckpt.r_ckpt[il]);
            }
        }
        if (s_l[il] != nullptr && ckpt.s_ckpt[il] != nullptr) {
            ggml_backend_t backend = find_backend_for_tensor(s_l[il], ckpt.backends);
            if (backend) {
                ggml_backend_tensor_copy_async(backend, backend, s_l[il], ckpt.s_ckpt[il]);
            } else {
                ggml_backend_tensor_copy(s_l[il], ckpt.s_ckpt[il]);
            }
        }
    }

    // No explicit sync needed - copies and subsequent graph compute are on the same backend streams
    // CUDA streams execute in FIFO order, so the copy will complete before graph compute starts
...
bool llama_memory_recurrent::checkpoint_restore(llama_seq_id seq_id) {
    ...
    if (ckpt.cell_idx < 0) {
        // checkpoint was empty (no state was saved)
        return true;
    }

    const int32_t tail_id = ckpt.cell_idx;

    // restore cell metadata
    cells[tail_id].pos = ckpt.pos;
    cells[tail_id].src = ckpt.src;
    cells[tail_id].seq_id = ckpt.seq_ids;
    cells[seq_id].tail = tail_id;
    head = ckpt.saved_head;
    used = ckpt.saved_used;

    // restore tensor data using GPU-to-GPU async copies
    const int32_t n_layer = hparams.n_layer;
    ...
    for (int32_t il = 0; il < n_layer; ++il) {
        if (r_l[il] != nullptr && ckpt.r_ckpt[il] != nullptr) {
            ggml_backend_t backend = find_backend_for_tensor(r_l[il], ckpt.backends);
            if (backend) {
                ggml_backend_tensor_copy_async(backend, backend, ckpt.r_ckpt[il], r_l[il]);
            } else {
                ggml_backend_tensor_copy(ckpt.r_ckpt[il], r_l[il]);
            }
        }
        if (s_l[il] != nullptr && ckpt.s_ckpt[il] != nullptr) {
            ggml_backend_t backend = find_backend_for_tensor(s_l[il], ckpt.backends);
            if (backend) {
                ggml_backend_tensor_copy_async(backend, backend, ckpt.s_ckpt[il], s_l[il]);
            } else {
                ggml_backend_tensor_copy(ckpt.s_ckpt[il], s_l[il]);
            }
        }
    }
