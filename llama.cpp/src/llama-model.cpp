        ...
        case LLM_ARCH_QWEN35:
            {
                ...
                // Mark recurrent layers (linear attention layers)
                {
                    uint32_t full_attn_interval = 4;
                    ml.get_key(LLM_KV_FULL_ATTENTION_INTERVAL, full_attn_interval, false);
                    // Only mark main model layers as recurrent, not MTP layers
                    const uint32_t n_main_layers = hparams.n_layer - hparams.nextn_predict_layers;
                    for (uint32_t i = 0; i < hparams.n_layer; ++i) {
                        if (i < n_main_layers) {
                            hparams.recurrent_layer_arr[i] = ((i + 1) % full_attn_interval != 0);
                        } else {
                            // MTP layers are standard attention, not recurrent
            ...
            case LLM_ARCH_QWEN35:
                {
                    ...
                    for (int i = 0; i < n_layer; ++i) {
                        ...
                        if (is_mtp_layer) {
                            // MTP layers: load norms + standard attention (gated, like full attention layers)
                            layer.attn_norm      = create_tensor(tn(LLM_TENSOR_ATTN_NORM,      "weight", i), { n_embd }, 0);
                            layer.attn_post_norm = create_tensor(tn(LLM_TENSOR_ATTN_POST_NORM, "weight", i), { n_embd }, 0);
                            ...
                            // Q/K normalization
                            layer.attn_q_norm = create_tensor(tn(LLM_TENSOR_ATTN_Q_NORM, "weight", i), { n_embd_head_k }, flags | TENSOR_NOT_REQUIRED);
                            layer.attn_k_norm = create_tensor(tn(LLM_TENSOR_ATTN_K_NORM, "weight", i), { n_embd_head_k }, flags | TENSOR_NOT_REQUIRED);
                            ...
                            // NextN/MTP specific tensors
                            layer.nextn.eh_proj          = create_tensor(tn(LLM_TENSOR_NEXTN_EH_PROJ,          "weight", i), { 2 * n_embd, n_embd }, flags);
                            layer.nextn.enorm            = create_tensor(tn(LLM_TENSOR_NEXTN_ENORM,            "weight", i), { n_embd }, flags);
                            layer.nextn.hnorm            = create_tensor(tn(LLM_TENSOR_NEXTN_HNORM,            "weight", i), { n_embd }, flags);

                            // Optional: dedicated embed_tokens (mtp_use_dedicated_embeddings=false means shared)
                            layer.nextn.embed_tokens     = create_tensor(tn(LLM_TENSOR_NEXTN_EMBED_TOKENS,     "weight", i), { n_embd, n_vocab }, flags | TENSOR_NOT_REQUIRED);
                            layer.nextn.shared_head_head = create_tensor(tn(LLM_TENSOR_NEXTN_SHARED_HEAD_HEAD, "weight", i), { n_embd, n_vocab }, flags | TENSOR_NOT_REQUIRED);
                            layer.nextn.shared_head_norm = create_tensor(tn(LLM_TENSOR_NEXTN_SHARED_HEAD_NORM, "weight", i), { n_embd }, flags | TENSOR_NOT_REQUIRED);
                        } else if (!hparams.is_recurrent(i)) {
                            // Full attention layers (main model)
                            layer.attn_norm      = create_tensor(tn(LLM_TENSOR_ATTN_NORM,      "weight", i), { n_embd }, 0);
                            layer.attn_post_norm = create_tensor(tn(LLM_TENSOR_ATTN_POST_NORM, "weight", i), { n_embd }, 0);
