...
struct server_slot {
    ...
    // MTP speculative checkpointing for hybrid (recurrent + attention) models
    // Saves recurrent state before speculative batch, restores after rejection
    int32_t mtp_checkpoint_prompt_len = 0;       // prompt length at checkpoint time
    bool mtp_checkpoint_valid = false;

    // Deferred correction: when drafts are rejected, instead of re-decoding immediately,
    // we fold the correction tokens into the next batch to avoid an extra forward pass.
    bool mtp_needs_correction = false;
    llama_tokens mtp_correction_tokens;  // tokens that need reprocessing through recurrent layers
    ...
    void reset() {
        ...
        n_draft_total = 0;
        n_draft_accepted = 0;
        mtp_checkpoint_valid = false;
        mtp_needs_correction = false;
        mtp_correction_tokens.clear();
...
private:
    ...
    void update_slots() {
        ...
        for (auto & slot : slots) {
            ...
            if (slot.mtp_needs_correction) {
                // Correction round: prepend correction tokens to update recurrent state,
                // then add slot.sampled normally. No speculation this round.
                // This avoids a costly extra forward pass by folding the correction into the
                // next regular decode.
                SLT_DBG(slot, "correction round: prepending %d tokens before sampled token\n",
                        (int) slot.mtp_correction_tokens.size());

                for (const auto tok : slot.mtp_correction_tokens) {
                    common_batch_add(batch, tok, slot.prompt.tokens.pos_next(), { slot.id }, false);
                    slot.prompt.tokens.push_back(tok);
                }
                slot.mtp_correction_tokens.clear();
                slot.mtp_needs_correction = false;

                // Add slot.sampled as normal non-speculative token (with logits)
                slot.i_batch = batch.n_tokens;
                common_batch_add(batch, slot.sampled, slot.prompt.tokens.pos_next(), { slot.id }, true);
                slot.prompt.tokens.push_back(slot.sampled);

                SLT_DBG(slot, "correction round decode, n_ctx = %d, n_tokens = %d\n",
                ...
                // Save recurrent state checkpoint before speculation (for hybrid models).
                // If any draft tokens are rejected, we restore the checkpoint and defer
                // correction to the next batch (avoiding an extra forward pass).
                slot.mtp_checkpoint_prompt_len = slot.prompt.n_tokens();
                slot.mtp_checkpoint_valid = llama_memory_checkpoint_save(
        ...
        for (int32_t i = 0; i < batch.n_tokens; i = i_next) {
            ...
            for (auto & slot : slots) {
                ...
                // For hybrid models with recurrent state: if any drafts were rejected,
                // restore the recurrent state checkpoint and defer correction to next round.
                // Instead of re-decoding immediately (costly extra forward pass), we save
                // the accepted tokens and prepend them to the next batch.
                const bool some_rejected = (ids.size() <= n_draft);
                if (slot.mtp_checkpoint_valid && some_rejected) {
                    // 1. Restore recurrent state to pre-speculation (direct tensor copy)
                    llama_memory_checkpoint_restore(llama_get_memory(ctx), slot.id);

                    // 2. Remove ALL speculative positions from memory (attention KV + recurrent no-op)
                    llama_memory_seq_rm(llama_get_memory(ctx), slot.id, slot.mtp_checkpoint_prompt_len, -1);

                    // 3. Extract correction tokens and revert prompt to checkpoint state
                    slot.mtp_correction_tokens.clear();
                    for (int32_t i = slot.mtp_checkpoint_prompt_len; i < slot.prompt.n_tokens(); i++) {
                        slot.mtp_correction_tokens.push_back(slot.prompt.tokens[i]);
                    }
                    slot.prompt.tokens.keep_first(slot.mtp_checkpoint_prompt_len);

                    slot.mtp_needs_correction = true;
                    slot.mtp_checkpoint_valid = false;

                    SLT_DBG(slot, "deferred correction: %d tokens to reprocess next round\n",
                            (int) slot.mtp_correction_tokens.size());
                } else {
                    // No rejections or no checkpoint - standard attention-only seq_rm
                    llama_memory_seq_rm(llama_get_memory(ctx), slot.id, slot.prompt.n_tokens(), -1);

                    // For MTP: set the correct hidden state to the last accepted position
                    if (slot.spec && ids.size() >= 2) {
                        llama_set_mtp_output_index(ctx, (int32_t)(ids.size() - 1));
                    }
                }
