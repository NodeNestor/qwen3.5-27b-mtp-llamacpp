...
// state of MTP (multi-token prediction) speculative decoding
//
// MTP uses the model's built-in prediction head (nextn_predict_layers) to draft tokens.
// Unlike the draft model approach, MTP shares the target model's context and weights.
// The MTP forward pass:
//   1. Take hidden state from main model's last layer output (pre-final-norm)
//   2. Apply hnorm (RMS norm) to hidden state
//   3. Apply enorm (RMS norm) to token embedding of the predicted token
//   4. Concatenate and project via eh_proj: intermediate = eh_proj(concat(normed_hidden, normed_embed))
//   5. Run through 1 transformer block (the MTP layer)
//   6. Apply shared_head_norm + shared_head_head (or shared lm_head) for draft logits
//
// Current status: placeholder implementation. Full MTP graph building requires:
//   - Saving pre-norm hidden state from main model decode (new field in llm_graph_result)
//   - Building a separate ggml graph for the MTP layer
//   - Evaluating it using the model's backend
//   - Extracting draft logits
// These changes span src/llama-graph.h, src/llama-context.cpp, and src/models/qwen35.cpp.
struct common_speculative_state_mtp : public common_speculative_state {
    ...
    void draft(
        ...
        // Chain the MTP layer iteratively to produce up to n_max draft tokens
        // Each iteration feeds the previous MTP hidden state + sampled draft token
        // back into the same MTP layer
        llama_token cur_token = id_last;
        const int32_t n_draft = std::max(1, params.n_max);

        for (int32_t i = 0; i < n_draft; ++i) {
            const int ret = llama_decode_mtp(ctx_tgt, cur_token);
            if (ret != 0) {
                LOG_DBG("MTP decode failed at iteration %d: %d\n", i, ret);
                break;
            }

            const float * logits = llama_get_logits_mtp(ctx_tgt);
            if (!logits) {
                break;
            }
